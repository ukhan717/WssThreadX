...
static UINT        _nx_dns_send_query_by_address(NX_DNS *dns_ptr, ULONG dns_server, UCHAR *ip_question, UCHAR *host_name_ptr, UINT host_name_buffer_size, ULONG wait_option);
static UINT        _nx_dns_send_query_get_rdata_by_name(NX_DNS *dns_ptr, ULONG server_address, UCHAR *host_name, UCHAR *record_buffer, 
...
UINT            status;
    ...
    memset(&dns_ptr -> nx_dns_server_ip_array[0], 0, NX_DNS_MAX_SERVERS * sizeof(ULONG));
    ...
    if(ip_ptr -> nx_ip_gateway_address != 0)
    {
        ...
        dns_ptr -> nx_dns_server_ip_array[0] = ip_ptr -> nx_ip_gateway_address; 
    }
...
UINT  _nx_dns_server_add(NX_DNS *dns_ptr, ULONG server_address)
{
    ...
    return(_nx_dns_server_add_internal(dns_ptr, server_address));
}
...
UINT        i;    
    ...
    while (i < NX_DNS_MAX_SERVERS) 
    {
        ...
        if(dns_ptr -> nx_dns_server_ip_array[i] != 0)
        {
            /* Is there a match? */
            if(dns_ptr -> nx_dns_server_ip_array[i] == server_address)
            {
                /* Error, release the mutex and return. */
                tx_mutex_put(&(dns_ptr -> nx_dns_mutex));

                /* Yes, no need to add ato the table, just return the 'error' status. */
    ...
    dns_ptr -> nx_dns_server_ip_array[i] = server_address;
...
UINT            found_match;
    ...
    while (i < NX_DNS_MAX_SERVERS - 1)
    {
        ...
        /* Move them up one slot. */
         ...
         DNSserver_array[i] = DNSserver_array[i+1];

        i++;
    }

    /* Terminate the last slot. */
    memset(&dns_ptr -> nx_dns_server_ip_array[NX_DNS_MAX_SERVERS - 1], 0, sizeof(ULONG));
...
UINT  _nx_dns_get_serverlist_size(NX_DNS *dns_ptr, UINT *size)
{
    ...
    *size = 0;
    ...
    while (*size < NX_DNS_MAX_SERVERS)
    {
        ...
        if (dns_ptr -> nx_dns_server_ip_array[*size] == 0)
        {
...
ULONG       server_address;
    ...
    *dns_server_address = server_address;
...
UINT            status;
    ...
    if (dns_ptr -> nx_dns_server_ip_array[index] == 0)
    {
...
UINT        i;
    ...
    *record_count = 0;
    ...
    if (dns_ptr -> nx_dns_server_ip_array[0] == 0)
    {
    ...
    for (retries = 0; retries < dns_ptr -> nx_dns_retries; retries++)
    {
        ...
        for (i = 0; (i < NX_DNS_MAX_SERVERS) && (dns_ptr -> nx_dns_server_ip_array[i] != 0); i ++)
        {
...
/*    nx_udp_socket_send                    Send DNS UDP packet           */ 
/*                                                                        */ 
/*  CALLED BY                                                             */ 
/*                                                                        */ 
/*    Application Code                                                    */ 
...
ULONG       rr_ttl;
    ...
    status =  nx_udp_socket_send(&dns_ptr -> nx_dns_socket, packet_ptr, dns_server, NX_DNS_PORT);
...
NX_PACKET           *receive_packet_ptr;
    ...
    /* Send the DNS packet out.  */
    status =  nx_udp_socket_send(&dns_ptr -> nx_dns_socket, packet_ptr, dns_server_address, NX_DNS_PORT);
...
UINT  _nx_dns_host_by_address_get(NX_DNS *dns_ptr, ULONG dns_address, UCHAR *host_name, UINT host_name_buffer_size, ULONG wait_option)
{
    ...
    return(_nx_dns_host_by_address_get_internal(dns_ptr, dns_address, host_name, host_name_buffer_size, wait_option));
}
