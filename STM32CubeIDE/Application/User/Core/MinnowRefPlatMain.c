...
sendLedInfo(ConnData* cd)
{
	int i, ledLen;
	SendData sd;
	const LedInfo* ledInfo = ledInfoPtr;
...
sendDeviceName(ConnData* cd)
{
	...
	JEncoder_beginArray(&sd.encoder);
	JEncoder_setString(&sd.encoder,"Name");
	JEncoder_endArray(&sd.encoder);
	return endMessage(&sd);
}
...
eventSimulator(ConnData* cd)
{
	...
	if( /* If a local button was pressed */sendSetLED(cd, x, on) )
	{
		return -1;
	}
	x = 7;
...
RecData_authenticate(RecData* o, ConnData* cd, JVal* v, JErr* e)
{
	...
	if( ! 0 )
	{
		 ...
		 */
		return sendLedInfo(cd) || sendSetTemp(cd, 1) ? -1: 0;
	}
	/* Not authenticated */
...
{
	printf("RecvData_runServer Called\r\n");
	MS* ms=cd->u.ms;
	...
	{ /* We get here if HTTP(S) was upgraded to a WebSocket con. */
		...
		printf("Closing WS connection: ecode = %d\n",rc);
	}
}
 ...
 */
static int
openServerSock(NX_IP *ip_ptr, NX_TCP_SOCKET *socket_ptr, CHAR *name,
		...
		UINT tcp_socket_size)
{
	...
	port=80;
#endif
	status = se_bind(ip_ptr, socket_ptr, name, NX_IP_NORMAL, NX_FRAGMENT_OKAY,
			NX_IP_TIME_TO_LIVE, 2048, NX_NULL, NX_NULL, sizeof(NX_TCP_SOCKET));
	if(status)
	{
		port= ALTERNATIVE_PORTNO;
		// TODO : The bind method here needs to be resolved
	...
	if(!status)
	{
		printf("openServerSock completed\r\n ");
	}
	return status;
}
 ...
 */
void
mainTask(SeCtx* ctx, NX_IP *ip_ptr, NX_TCP_SOCKET *socket_ptr, CHAR *name,
		ULONG type_of_service, ULONG fragment, UINT time_to_live, ULONG window_size,
		VOID (*tcp_urgent_data_callback)(NX_TCP_SOCKET *socket_ptr),
		VOID (*tcp_disconnect_callback)(NX_TCP_SOCKET *socket_ptr),
		UINT tcp_socket_size)
{
	...
	wph.fetchPage = fetchPage; // TODO : Undo this maybe??


	// TODO : UNDO the lower part
	if(openServerSock(ip_ptr, socket_ptr, name, NX_IP_NORMAL, NX_FRAGMENT_OKAY,
			NX_IP_TIME_TO_LIVE, 512, NX_NULL, NX_NULL, sizeof(NX_TCP_SOCKET)))
	{
		return;
	}
	else
	...
	listenSockPtr->nxSock = *socket_ptr; // Check for the values here - Or pass by reference
	int conditionTrue = 5;
	...
	for(;;)
	{
		...
		conditionTrue = se_accept(ip_ptr, &listenSockPtr, NX_WAIT_FOREVER, &sockPtr);

		/* get the socket state */
		nx_tcp_socket_info_get(socket_ptr, NULL, NULL, NULL, NULL, NULL, NULL, NULL, &socket_state, NULL, NULL, NULL);
		...
		//		switch(se_accept(socket_ptr, &listenSockPtr, NX_WAIT_FOREVER, &sockPtr))
		...
		case 1: /* Accepted new client connection i.e. new browser conn. */
		{
			...
			if(ret == NX_SUCCESS)
			{
				...
				ret = nx_tcp_socket_receive(socket_ptr, &data_packet, NX_WAIT_FOREVER);
				...
				if (ret == NX_SUCCESS)
				{
					...
					printf("Recieved Data :  %lu,%u,%s \n",source_ip_address, source_port, data_buffer);

					if (ret == NX_SUCCESS)
					{
						printf("Data Echo succeeded\r\n");
					}
				}
				else
				{
					nx_tcp_socket_disconnect(socket_ptr, NX_WAIT_FOREVER);
					nx_tcp_server_socket_unaccept(socket_ptr);
					nx_tcp_server_socket_relisten(ip_ptr, 80, socket_ptr);
				}
			}
			else
			{
				/*toggle the green led to indicate the idle state */
				printf("toggle the green led to indicate the idle state \r\n");
			}
...
/*Splitting MainTask into Two Subtasks*/

UINT minorTask(SeCtx* ctx, NX_IP *ip_ptr, NX_TCP_SOCKET *socket_ptr, ULONG wait_option)
{
	static WssProtocolHandshake wph={0};
	static ConnData cd;
	static RecData rd;
